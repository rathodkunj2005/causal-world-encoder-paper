...
\title{Causal World Encoder: Real-Data Small-Scale Tests of Action Decomposition, Timescale Compression, and Sparse Commitment}
...
\begin{abstract}
This paper tests a narrow architectural claim: agent world models benefit when action effects are separated from passive dynamics, history is compressed with explicit temporal structure, and prediction updates can be made sparse. We study a proposed Causal World Encoder (CWE) with three components: a hierarchical timescale belief state (HTBS), a twin-stream intervention-conditioned predictor (TICP), and a sparse predictive commitment gate (SPCG). Unlike many architecture proposals, the paper uses only real data and reproducible small-scale experiments. On 1,986 LoCoMo questions over 5,882 dialogue memories, lexical retrieval remains strongest at Hits@5 (0.3197) and the hybrid ranker only narrowly improves Hits@10 (0.3933 vs. 0.3922), which is a useful reminder that long-horizon memory often depends on exact evidence spans. On an eight-document workflow built from live arXiv abstracts, bounded workspace reconstruction reduces cumulative prompt load by 69.23\% while preserving full title and keyword coverage. On BabyAI offline trajectories, the twin-stream predictor improves held-out-action cosine from 0.4046 to 0.4224 and local Top-1 from 0.3003 to 0.3271. The timescale experiment is less flattering to the new architecture: HTBS is competitive but not dominant, while transformer or even per-step MLP baselines often win on local decode. The sparse gate hits its target commitment rate (30.00\%) and the committed subset reaches Top-1 0.5195, but its preference for high-change steps is weak (1.28\% gap). The overall conclusion is therefore mixed and honest: explicit action/world decomposition helps, bounded external state helps, but the present HTBS and SPCG designs do not yet justify broad claims of superiority over simpler baselines.
\end{abstract}

\section{Introduction}
Language-model agents still inherit two stubborn architectural problems. First, most sequence models learn primarily from observation rather than intervention. They can compress correlations in trajectories, but they do not naturally separate what the environment would do on its own from what changed because the agent acted. Second, they typically access history either by replaying a growing prompt or by compressing everything into an opaque recurrent state. Neither extreme is satisfying for long-horizon agents that need persistent memory, bounded context, and explicit support for planning.

This paper evaluates a design hypothesis rather than a benchmark-chasing system. The proposed Causal World Encoder (CWE) combines three ideas. HTBS organizes memory into slots that update on different clocks. TICP decomposes next-state prediction into a state-only dynamics term and an action-conditioned residual. SPCG allows the model to defer costly predictive updates unless a state is worth committing. The intellectual center of gravity is TICP, because it is the only component that directly addresses the observation-versus-intervention mismatch.

The empirical stance is deliberately conservative. Instead of claiming a general alternative to transformers or a full structural causal model, the paper asks which architectural biases survive contact with real data on a laptop. The answer is nuanced. Some ideas work. Some mostly do not. That is still useful.

\paragraph{Contributions.}
\begin{enumerate}
    \item We formalize CWE as a modular extension of bounded-state agent architecture, separating multiscale belief compression, action-effect decomposition, and sparse predictive commitment.
    \item We run five real-data experiments: LoCoMo retrieval, a live arXiv bounded-workspace benchmark, held-out-action generalization on BabyAI trajectories, sequence-model comparison across temporal horizons, and sparse commitment analysis.
    \item We report a mixed result set rather than smoothing it into a marketing story: twin-stream action decomposition helps, bounded workspace helps, lexical retrieval remains surprisingly strong, and the proposed HTBS/SPCG components do not yet dominate simpler baselines.
\end{enumerate}
...
\midrule
4 & MLP control & 0.3327 & 0.6041 & 2.65 \\
8 & Transformer & 0.3478 & 0.6500 & 33.66 \\
12 & MLP control & 0.3542 & 0.7205 & 2.06 \\
16 & Transformer & 0.4369 & 0.8702 & 14.17 \\
\bottomrule
\end{tabular}
...
Sparse blended output & 0.6147 & 0.3284 & 0.7229 \\
Sparse committed subset & 0.5920 & 0.5195 & 0.9123 \\
\bottomrule
\end{tabular}
\label{tab:sparse}
\end{table}
...
Statistic & Value \\
\midrule
Mean gate probability & 0.3060 \\
Actual commitment rate & 0.3000 \\
High-change commitment rate & 0.3079 \\
Low-change commitment rate & 0.2951 \\
Committed Top-1 minus dense Top-1 & 0.1842 \\
\bottomrule
\end{tabular}
...
SPCG reaches the target average budget almost exactly, and the committed subset attains Top-1 0.5195 versus 0.3353 for the dense baseline, a gain of 0.1842. That is the good news. The bad news is that the gate barely distinguishes high-change from low-change transitions: the commitment-rate gap is only 1.28\%. So the compute-allocation story looks real, but the stronger event-boundary interpretation is weak.
...
Second, explicit action/world decomposition helps. The held-out-action BabyAI experiment is not a giant effect, but it is the most coherent positive finding in the repo. It justifies taking TICP seriously as the core novel component.
